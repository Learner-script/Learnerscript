 ...
 */
namespace block_learnerscript\reports\useractivities;
...
class report extends reportbase implements \block_learnerscript\report {
    ...
    public function where() {
        ...
        $this->sql .= " AND m.visible = 1 AND (CASE WHEN main.availability IS NOT NULL THEN concat(main.id, '-', u.id) IN(
                            SELECT concat(cm.id, '-', gm.userid)
                            FROM {course_modules} cm
                            JOIN {groups} g ON g.courseid = cm.course
                            JOIN {groups_members} gm ON gm.groupid=g.id
                            WHERE cm.visible = 1 AND cm.availability LIKE CONCAT('%\"type\":\"group\",\"id\":', g.id, '%'))
                            ELSE main.availability IS NULL END) ";
