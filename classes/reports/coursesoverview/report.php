 ...
 */
namespace block_learnerscript\reports\coursesoverview;
...
class report extends reportbase implements \block_learnerscript\report {
    ...
    public function column_queries($columnname, $courseid) {
        ...
        $concatsql .= " AND (CASE WHEN cm.availability IS NOT NULL THEN cm.id IN(
                        SELECT cmd.id
                        FROM {course_modules} cmd
                        JOIN {groups} g ON g.courseid = cmd.course
                        JOIN {groups_members} gm ON gm.groupid=g.id
                        WHERE gm.userid = $filteruserid AND cmd.visible = 1
            ...
            case 'grades' :
                ...
                $gradesql = "SELECT CASE WHEN SUM(gi.grademax) > 0
                    ...
                    ELSE '--' END
                              ...
                              WHERE gg.userid = $filteruserid  $concatsql $where ";
                if (!empty($filtermoduleid)) {
