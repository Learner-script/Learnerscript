 ...
 */
namespace block_learnerscript\reports\usercourses;
use block_learnerscript\local\reportbase;
use context_course;

/**
 * User courses report class
 */
class report extends reportbase implements \block_learnerscript\report {
    ...
    public function column_queries($columnname, $usercourseid) {
        ...
        $identity = " ";
        $concatsql = '';
        $concatsql .= " AND (CASE WHEN cm.availability IS NOT NULL THEN concat(cm.id,'-',cmc.userid) IN(
                        SELECT concat(cmd.id,'-',gm.userid)
                        FROM {course_modules} cmd
                        JOIN {groups} g ON g.courseid = cmd.course
                        JOIN {groups_members} gm ON gm.groupid=g.id
                        WHERE cmd.visible = 1 AND cmd.availability LIKE CONCAT('%\"type\":\"group\",\"id\":', g.id, '%'))
                        ELSE cm.availability IS NULL END) ";
        switch ($columnname) {
            case 'grade':
                $identity = 'gg.userid';
            ...
            case 'completedassignments':
                ...
                $query = "SELECT COUNT(cm.id) AS completedassignments
                                       ...
                                       WHERE m.name = 'assign' AND cm.visible = 1 AND cm.deletioninprogress = 0
                                         AND cm.course = $filtercourseid AND cmc.completionstate != 0 $where $concatsql";
            break;
            ...
            case 'completedactivities':
                ...
                $query = "SELECT COUNT(cm.id) AS completedactivities
                                       ...
                                       WHERE  cm.visible = 1 AND cm.deletioninprogress = 0 AND cm.course = $filtercourseid
                                         AND cmc.completionstate != 0 $where $concatsql";
            break;
