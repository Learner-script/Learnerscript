 ...
 */
namespace block_learnerscript\reports\userprofile;
...
class report extends reportbase {
    ...
    public function column_queries($column, $userid) {
        ...
        $concatsql = " ";
        if ($this->role == $this->currentrole) {
            $concatsql .= " AND (CASE WHEN cm.availability IS NOT NULL THEN concat(cm.id, '-', ra.userid) IN(
        ...
        switch ($column) {
            ...
            case 'assignments':
                ...
                $query = "SELECT COUNT(cm.id) AS assignments
                                ...
                                JOIN {role} r ON r.id = ra.roleid AND r.shortname = 'student'
                                WHERE m.name = 'assign'
                                AND cm.visible = 1 AND cm.deletioninprogress = 0 $where $coursefilter $concatsql ";
                break;
            case 'quizes':
                $identity = 'ra.userid';
