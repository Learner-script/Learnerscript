...
$params['pagetypepattern'] = '%blocks-reportdashboard%';
$blockinstances = $DB->get_records_sql("SELECT *
FROM {block_instances}
WHERE " . $DB->sql_like('pagetypepattern', ':pagetypepattern', false),
$params);
foreach ($blockinstances as $bi) {
    ...
    if (isset($configdata->reportlist) && $configdata->reportlist == $id) {
        ...
        $data .= "</instance>";
        $blockposition = $DB->get_record_sql('SELECT *
        FROM {block_positions}
        WHERE blockinstanceid = :blockinstanceid', ['blockinstanceid' => $bi->id]);
